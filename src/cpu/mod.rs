...
impl Cpu {
    ...
    pub fn step(&mut self, mmu: &mut Mmu, timer: &mut Timer) -> u32 {
        ...
        match opcode {
            ...
            // ==========================================
            0x01 => {
                let bc = self.ld_r16_imm16(mmu, timer);
                set_u16register!(self, self.b, self.c, bc);
            }
            0x11 => {
                let de = self.ld_r16_imm16(mmu, timer);
                set_u16register!(self, self.d, self.e, de);
            }
            0x21 => {
                let hl = self.ld_r16_imm16(mmu, timer);
                set_u16register!(self, self.h, self.l, hl);
            }
            0x31 => self.sp = self.ld_r16_imm16(mmu, timer),
            ...
            0x34 => self.inc_hl_mem(mmu, timer), // Speciale: incrementa la memoria puntata da (HL)
            ...
            0x35 => self.dec_hl_mem(mmu, timer), // Speciale: decrementa la memoria puntata da (HL)
            0x3D => self.a = self.dec_r8(self.a),

            // ==========================================
            // CARICAMENTI IMMEDIATI A 8 BIT (LD r8, n8)
            // ==========================================
            0x06 => self.b = self.ld_r8_imm8(mmu, timer),
            0x0E => self.c = self.ld_r8_imm8(mmu, timer),
            0x16 => self.d = self.ld_r8_imm8(mmu, timer),
            0x1E => self.e = self.ld_r8_imm8(mmu, timer),
            0x26 => self.h = self.ld_r8_imm8(mmu, timer),
            0x2E => self.l = self.ld_r8_imm8(mmu, timer),
            0x36 => self.ld_hl_mem_imm8(mmu, timer), // LD (HL), n8
            0x3E => self.a = self.ld_r8_imm8(mmu, timer),
            ...
            0x02 => {
                let bc = get_u16register!(self, self.b, self.c);
                self.ld_mem_r16_a(mmu, timer, bc)
            } // LD (BC), A
            0x12 => {
                let de = get_u16register!(self, self.d, self.e);
                self.ld_mem_r16_a(mmu, timer, de)
            } // LD (DE), A
            0x0A => {
                let bc = get_u16register!(self, self.b, self.c);
                self.ld_a_mem_r16(mmu, timer, bc)
            } // LD A, (BC)
            0x1A => {
                let de = get_u16register!(self, self.d, self.e);
                self.ld_a_mem_r16(mmu, timer, de)
            } // LD A, (DE)
            0x08 => self.ld_mem16_sp(mmu, timer), // LD (n16), SP

            // Auto-incremento / decremento HL
            0x22 => self.ld_hl_inc_a(mmu, timer), // LD (HL+), A
            0x2A => self.ld_a_hl_inc(mmu, timer), // LD A, (HL+)
            0x32 => self.ld_hl_dec_a(mmu, timer), // LD (HL-), A
            0x3A => self.ld_a_hl_dec(mmu, timer), // LD A, (HL-)

            // ==========================================
            // SALTI RELATIVI CONDIZIONATI (JR)
            // ==========================================
            0x18 => self.jr_cond(true, mmu, timer),
            0x20 => self.jr_cond(!self.get_flag_z(), mmu, timer), // JR NZ, e8
            0x28 => self.jr_cond(self.get_flag_z(), mmu, timer),  // JR Z, e8
            0x30 => self.jr_cond(!self.get_flag_c(), mmu, timer), // JR NC, e8
            0x38 => self.jr_cond(self.get_flag_c(), mmu, timer),  // JR C, e8
            ...
            0x46 => self.b = self.ld_r8_mem_hl(mmu, timer), // LD B, (HL)
            ...
            0x4E => self.c = self.ld_r8_mem_hl(mmu, timer), // LD C, (HL)
            ...
            0x56 => self.d = self.ld_r8_mem_hl(mmu, timer), // LD D, (HL)
            ...
            0x5E => self.e = self.ld_r8_mem_hl(mmu, timer), // LD E, (HL)
            ...
            0x66 => self.h = self.ld_r8_mem_hl(mmu, timer), // LD H, (HL)
            ...
            0x6E => self.l = self.ld_r8_mem_hl(mmu, timer), // LD L, (HL)
            ...
            0x70 => self.ld_mem_hl_r8(mmu, timer, self.b),
            0x71 => self.ld_mem_hl_r8(mmu, timer, self.c),
            0x72 => self.ld_mem_hl_r8(mmu, timer, self.d),
            0x73 => self.ld_mem_hl_r8(mmu, timer, self.e),
            0x74 => self.ld_mem_hl_r8(mmu, timer, self.h),
            0x75 => self.ld_mem_hl_r8(mmu, timer, self.l),
            0x77 => self.ld_mem_hl_r8(mmu, timer, self.a),
            ...
            0x7E => self.a = self.ld_r8_mem_hl(mmu, timer), // LD A, (HL)
            ...
            0x86 => self.alu_add_mem_hl(mmu, timer),
            ...
            0x8E => self.alu_adc_mem_hl(mmu, timer),
            ...
            0x96 => self.alu_sub_mem_hl(mmu, timer),
            ...
            0x9E => self.alu_sbc_mem_hl(mmu, timer),
            ...
            0xA6 => self.alu_and_mem_hl(mmu, timer),
            ...
            0xAE => self.alu_xor_mem_hl(mmu, timer),
            ...
            0xB6 => self.alu_or_mem_hl(mmu, timer),
            ...
            0xBE => self.alu_cp_mem_hl(mmu, timer),
            ...
            // POP & PUSH
            0xC1 => {
                let bc = self.pop_r16(mmu, timer);
                set_u16register!(self, self.b, self.c, bc);
            }
            0xD1 => {
                let de = self.pop_r16(mmu, timer);
                set_u16register!(self, self.d, self.e, de);
            }
            0xE1 => {
                let hl = self.pop_r16(mmu, timer);
                set_u16register!(self, self.h, self.l, hl);
            }
            0xF1 => {
                let af = self.pop_r16(mmu, timer);
                self.a = ((af >> 8) & 0xFF) as u8;
                self.f = (af & 0xF0) as u8;
            }

            0xC5 => self.push_r16(self.b, self.c, mmu, timer),
            0xD5 => self.push_r16(self.d, self.e, mmu, timer),
            0xE5 => self.push_r16(self.h, self.l, mmu, timer),
            0xF5 => self.push_af(mmu, timer),

            // RET Condizionati ed Incondizionati
            0xC0 => self.ret_cond(!self.get_flag_z(), mmu, timer),
            0xC8 => self.ret_cond(self.get_flag_z(), mmu, timer),
            0xD0 => self.ret_cond(!self.get_flag_c(), mmu, timer),
            0xD8 => self.ret_cond(self.get_flag_c(), mmu, timer),
            0xC9 => self.ret_incond(mmu, timer),
            0xD9 => self.reti(mmu, timer),

            // JP Condizionati ed Incondizionati
            0xC2 => self.jp_cond(!self.get_flag_z(), mmu, timer),
            0xCA => self.jp_cond(self.get_flag_z(), mmu, timer),
            0xD2 => self.jp_cond(!self.get_flag_c(), mmu, timer),
            0xDA => self.jp_cond(self.get_flag_c(), mmu, timer),
            0xC3 => self.jp(mmu, timer),
            0xE9 => self.jp_hl(),

            // CALL Condizionati ed Incondizionati
            0xC4 => self.call_cond(!self.get_flag_z(), mmu, timer),
            0xCC => self.call_cond(self.get_flag_z(), mmu, timer),
            0xD4 => self.call_cond(!self.get_flag_c(), mmu, timer),
            0xDC => self.call_cond(self.get_flag_c(), mmu, timer),
            0xCD => self.call(mmu, timer),

            // RESTART (RST)
            0xC7 => self.rst(0x00, mmu, timer),
            0xCF => self.rst(0x08, mmu, timer),
            0xD7 => self.rst(0x10, mmu, timer),
            0xDF => self.rst(0x18, mmu, timer),
            0xE7 => self.rst(0x20, mmu, timer),
            0xEF => self.rst(0x28, mmu, timer),
            0xF7 => self.rst(0x30, mmu, timer),
            0xFF => self.rst(0x38, mmu, timer),

            // ==========================================
            // OPERAZIONI ALU IMMEDIATE (Valori a 8 bit)
            // ==========================================
            0xC6 => self.add_a_imm8(mmu, timer),
            0xCE => self.adc_a_imm8(mmu, timer),
            0xD6 => self.sub_a_imm8(mmu, timer),
            0xDE => self.sbc_a_imm8(mmu, timer),
            0xE6 => self.and_a_imm8(mmu, timer),
            0xEE => self.xor_a_imm8(mmu, timer),
            0xF6 => self.or_a_imm8(mmu, timer),
            0xFE => self.cp_a_imm8(mmu, timer),

            // ==========================================
            // CARICAMENTI SPECIALI / RAM ALTA (LDH)
            // ==========================================
            0xE0 => self.ldh_mem8_a(mmu, timer), // LDH (n8), A
            0xF0 => self.ldh_a_mem8(mmu, timer), // LDH A, (n8)
            0xE2 => self.ld_mem_c_a(mmu, timer), // LD (C), A
            0xF2 => self.ld_a_mem_c(mmu, timer), // LD A, (C)
            0xEA => self.ld_mem16_a(mmu, timer), // LD (n16), A
            0xFA => self.ld_a_mem16(mmu, timer), // LD A, (n16)

            // Manipolazioni SP
            0xE8 => self.add_sp_e8(mmu, timer),
            0xF8 => self.ld_hl_sp_e8(mmu, timer),
            0xF9 => self.ld_sp_hl(),
    ...
    fn cb_prefixed(&mut self, cb_opcode: u8, mmu: &mut Mmu, timer: &mut Timer) {
        match cb_opcode {
            // 0x00..0x07: RLC
            ...
            0x06 => self.rlc_hl_mem(mmu, timer),
            ...
            0x0E => self.rrc_hl_mem(mmu, timer),
            ...
            0x26 => self.sla_hl_mem(mmu, timer),
            ...
            0x2E => self.sra_hl_mem(mmu, timer),
            ...
            0x36 => self.swap_hl_mem(mmu, timer),
            ...
            0x3E => self.srl_hl_mem(mmu, timer),
            ...
            0x46 => self.bit_b_hl_mem(0, mmu, timer),
            ...
            0x4E => self.bit_b_hl_mem(1, mmu, timer),
            ...
            0x56 => self.bit_b_hl_mem(2, mmu, timer),
            ...
            0x5E => self.bit_b_hl_mem(3, mmu, timer),
            ...
            0x66 => self.bit_b_hl_mem(4, mmu, timer),
            ...
            0x6E => self.bit_b_hl_mem(5, mmu, timer),
            ...
            0x76 => self.bit_b_hl_mem(6, mmu, timer),
            ...
            0x7E => self.bit_b_hl_mem(7, mmu, timer),
            ...
            0x86 => self.res_b_hl_mem(0, mmu, timer),
            ...
            0x8E => self.res_b_hl_mem(1, mmu, timer),
            ...
            0x96 => self.res_b_hl_mem(2, mmu, timer),
            ...
            0x9E => self.res_b_hl_mem(3, mmu, timer),
            ...
            0xA6 => self.res_b_hl_mem(4, mmu, timer),
            ...
            0xAE => self.res_b_hl_mem(5, mmu, timer),
            ...
            0xB6 => self.res_b_hl_mem(6, mmu, timer),
            ...
            0xBE => self.res_b_hl_mem(7, mmu, timer),
            ...
            0xC6 => self.set_b_hl_mem(0, mmu, timer),
            ...
            0xCE => self.set_b_hl_mem(1, mmu, timer),
            ...
            0xD6 => self.set_b_hl_mem(2, mmu, timer),
            ...
            0xDE => self.set_b_hl_mem(3, mmu, timer),
            ...
            0xE6 => self.set_b_hl_mem(4, mmu, timer),
            ...
            0xEE => self.set_b_hl_mem(5, mmu, timer),
            ...
            0xF6 => self.set_b_hl_mem(6, mmu, timer),
