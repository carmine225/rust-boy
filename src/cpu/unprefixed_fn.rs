...
impl Cpu {
    ...
    pub fn dec_hl_mem(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let hl = get_u16register!(self, self.h, self.l);
        let value = mmu.read_byte(hl, timer);
        let result = value.wrapping_sub(1);
        mmu.write_byte(hl, result, timer);
    ...
    pub fn ld_hl_mem_imm8(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let hl = get_u16register!(self, self.h, self.l);
        let value = mmu.read_byte(self.pc, timer);
        mmu.write_byte(hl, value, timer);
        self.pc = self.pc.wrapping_add(1);
        self.cycles = self.cycles.wrapping_add(12);
    }
    ...
    /// Modifica i flag Z, N, e H. Il flag C non viene modificato.
    pub fn inc_hl_mem(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let hl = get_u16register!(self, self.h, self.l);
        let value = mmu.read_byte(hl, timer);
    ...
    pub fn ld_mem_r16_a(&mut self, mmu: &mut Mmu, timer: &mut Timer, src: u16) {
        mmu.write_byte(src, self.a, timer);
        self.cycles = self.cycles.wrapping_add(8);
    }
    ...
    pub fn ld_a_mem_r16(&mut self, mmu: &mut Mmu, timer: &mut Timer, src: u16) {
        self.a = mmu.read_byte(src, timer);
        self.cycles = self.cycles.wrapping_add(8);
    }
    ...
    pub fn ld_mem16_sp(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let low = mmu.read_byte(self.pc, timer) as u16;
        let high = mmu.read_byte(self.pc.wrapping_add(1), timer) as u16;
        let addr = low | (high << 8);
        mmu.write_byte(addr, self.sp as u8, timer);
        mmu.write_byte(addr.wrapping_add(1), (self.sp >> 8) as u8, timer);
    ...
    pub fn ld_hl_inc_a(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let hl = get_u16register!(self, self.h, self.l);
        mmu.write_byte(hl, self.a, timer);
    ...
    pub fn ld_a_hl_inc(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let hl = get_u16register!(self, self.h, self.l);
        self.a = mmu.read_byte(hl, timer);
        let hl = hl.wrapping_add(1);
        self.h = (hl >> 8) as u8;
        self.l = (hl & 0xFF) as u8;
        self.cycles = self.cycles.wrapping_add(8); // Aggiorna il conteggio dei cicli in base all'operazione
    ...
    pub fn ld_hl_dec_a(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let hl = get_u16register!(self, self.h, self.l);
        mmu.write_byte(hl, self.a, timer);
    ...
    pub fn ld_a_hl_dec(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let hl = get_u16register!(self, self.h, self.l);
        self.a = mmu.read_byte(hl, timer);
        let hl = hl.wrapping_sub(1);
        self.h = (hl >> 8) as u8;
    ...
    pub fn ld_mem_hl_r8(&mut self, mmu: &mut Mmu, timer: &mut Timer, src: u8) {
        let hl = get_u16register!(self, self.h, self.l);
        mmu.write_byte(hl, src, timer);
        self.cycles = self.cycles.wrapping_add(8); // Aggiorna il conteggio dei cicli in base all'operazione
    }
    ...
    pub fn ld_r8_mem_hl(&mut self, mmu: &mut Mmu, timer: &mut Timer) -> u8 {
        let hl = get_u16register!(self, self.h, self.l);
        let data = mmu.read_byte(hl, timer);
        self.cycles = self.cycles.wrapping_add(8);
        data
    }
    ...
    pub fn alu_add_mem_hl(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let hl = get_u16register!(self, self.h, self.l);
        let value = mmu.read_byte(hl, timer);
    ...
    /// Modifica i flag Z, N, H, e C.
    pub fn alu_sub_mem_hl(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let hl = get_u16register!(self, self.h, self.l);
        let value = mmu.read_byte(hl, timer);
    ...
    /// Modifica i flag Z, N, H, e C.
    pub fn alu_cp_mem_hl(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let hl = get_u16register!(self, self.h, self.l);
        let value = mmu.read_byte(hl, timer);
    ...
    /// Modifica i flag Z, N, H, e C.
    pub fn alu_adc_mem_hl(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let hl = get_u16register!(self, self.h, self.l);
        let value = mmu.read_byte(hl, timer);
        let carry = if self.get_flag_c() { 1 } else { 0 };
    ...
    /// Modifica i flag Z, N, H, e C.
    pub fn alu_sbc_mem_hl(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let hl_addr = get_u16register!(self, self.h, self.l);
        let value = mmu.read_byte(hl_addr, timer);
        let carry = if self.get_flag_c() { 1 } else { 0 };
        let a = self.a;
    ...
    pub fn alu_and_mem_hl(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let hl = get_u16register!(self, self.h, self.l);
        let value = mmu.read_byte(hl, timer);
        self.a &= value;
        self.set_flag_z(self.a == 0);
    ...
    pub fn alu_xor_mem_hl(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let hl = get_u16register!(self, self.h, self.l);
        let value = mmu.read_byte(hl, timer);
        self.a ^= value;
        self.set_flag_z(self.a == 0);
    ...
    /// Azzera i flag N, H, C. Modifica Z.
    pub fn alu_or_mem_hl(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let hl = get_u16register!(self, self.h, self.l);
        let value = mmu.read_byte(hl, timer);
        self.a |= value;
        self.set_flag_z(self.a == 0);
    ...
    pub fn ld_r16_imm16(&mut self, mmu: &mut Mmu, timer: &mut Timer) -> u16 {
        let value_low = mmu.read_byte(self.pc, timer);
        self.pc = self.pc.wrapping_add(1);
        let value_high = mmu.read_byte(self.pc, timer);
    ...
    /// Incrementa il PC di 1.
    pub fn ld_r8_imm8(&mut self, mmu: &mut Mmu, timer: &mut Timer) -> u8 {
        let value = mmu.read_byte(self.pc, timer);
        self.pc = self.pc.wrapping_add(1);
        self.cycles = self.cycles.wrapping_add(8);
        value
    ...
    pub fn jr_cond(&mut self, condition: bool, mmu: &mut Mmu, timer: &mut Timer) {
        let offset_raw = mmu.read_byte(self.pc, timer);
        self.pc = self.pc.wrapping_add(1);
    ...
    /// Incrementa SP di 2.
    pub fn pop_r16(&mut self, mmu: &mut Mmu, timer: &mut Timer) -> u16 {
        let low = mmu.read_byte(self.sp, timer);
        self.sp = self.sp.wrapping_add(1);
        let high = mmu.read_byte(self.sp, timer);
        self.sp = self.sp.wrapping_add(1);
        self.cycles = self.cycles.wrapping_add(12);
    ...
    pub fn push_r16(&mut self, high: u8, low: u8, mmu: &mut Mmu, timer: &mut Timer) {
    ...
    /// Decrementa SP di 2.
    pub fn push_af(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let high = self.a;
        let low = self.f & 0xF0; // I flag sono solo i 4 bit più significativi
        self.push_r16(high, low, mmu, timer);
    }
    ...
    /// Se la condizione è falsa, continua l'esecuzione sequenziale.
    pub fn ret_cond(&mut self, _condition: bool, mmu: &mut Mmu, timer: &mut Timer) {
        if _condition {
    ...
    pub fn ret_incond(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        self.pc = self.pop_r16(mmu, timer);
        self.cycles = self.cycles.wrapping_add(4);
    }
    ...
    /// Se la condizione è falsa, continua l'esecuzione sequenziale.
    pub fn jp_cond(&mut self, _condition: bool, mmu: &mut Mmu, timer: &mut Timer) {
        let addr_low = mmu.read_byte(self.pc, timer);
        self.pc = self.pc.wrapping_add(1);
        let addr_high = mmu.read_byte(self.pc, timer);
        self.pc = self.pc.wrapping_add(1);
    ...
    pub fn jp(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let addr_low = mmu.read_byte(self.pc, timer);
        self.pc = self.pc.wrapping_add(1);
        let addr_high = mmu.read_byte(self.pc, timer);
        self.pc = self.pc.wrapping_add(1);
    ...
    pub fn call_cond(&mut self, _condition: bool, mmu: &mut Mmu, timer: &mut Timer) {
        if _condition {
            self.call(mmu, timer);
        } else {
    ...
    /// Decrementa SP di 2.
    pub fn call(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let addr_low = mmu.read_byte(self.pc, timer);
        self.pc = self.pc.wrapping_add(1);
        let addr_high = mmu.read_byte(self.pc, timer);
        self.pc = self.pc.wrapping_add(1);
        ...
        self.push_r16(pc_high, pc_low, mmu, timer);
    ...
    pub fn rst(&mut self, target_addr: u16, mmu: &mut Mmu, timer: &mut Timer) {
        let pc_high = (self.pc >> 8) as u8;
        let pc_low = (self.pc & 0xFF) as u8;
        self.push_r16(pc_high, pc_low, mmu, timer);
        self.pc = target_addr;
    ...
    pub fn add_a_imm8(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let value = mmu.read_byte(self.pc, timer);
        self.alu_add(value);
        self.pc = self.pc.wrapping_add(1);
        self.cycles = self.cycles.wrapping_add(4);
    ...
    /// Incrementa PC di 1. Modifica i flag Z, N, H, e C.
    pub fn adc_a_imm8(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let value = mmu.read_byte(self.pc, timer);
        self.alu_adc(value);
        self.pc = self.pc.wrapping_add(1);
        self.cycles = self.cycles.wrapping_add(4);
    ...
    /// Incrementa PC di 1. Modifica i flag Z, N, H, e C.
    pub fn sub_a_imm8(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let value = mmu.read_byte(self.pc, timer);
        self.alu_sub(value);
        self.pc = self.pc.wrapping_add(1);
        self.cycles = self.cycles.wrapping_add(4);
    ...
    /// Incrementa PC di 1. Modifica i flag Z, N, H, e C.
    pub fn sbc_a_imm8(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let value = mmu.read_byte(self.pc, timer);
        self.alu_sbc(value);
        self.pc = self.pc.wrapping_add(1);
        self.cycles = self.cycles.wrapping_add(4);
    ...
    /// Incrementa PC di 1.
    pub fn add_sp_e8(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let raw_offset = mmu.read_byte(self.pc, timer);
        self.pc = self.pc.wrapping_add(1);
    ...
    pub fn and_a_imm8(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let value = mmu.read_byte(self.pc, timer);
        self.alu_and(value);
        self.pc = self.pc.wrapping_add(1);
        self.cycles = self.cycles.wrapping_add(4);
    ...
    pub fn xor_a_imm8(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let value = mmu.read_byte(self.pc, timer);
        self.alu_xor(value);
        self.pc = self.pc.wrapping_add(1);
        self.cycles = self.cycles.wrapping_add(4);
    ...
    /// Incrementa PC di 1. Azzera i flag N, H, C. Modifica Z.
    pub fn or_a_imm8(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let value = mmu.read_byte(self.pc, timer);
        self.alu_or(value);
        self.pc = self.pc.wrapping_add(1);
        self.cycles = self.cycles.wrapping_add(4);
    ...
    /// Incrementa PC di 1. Modifica i flag Z, N, H, e C.
    pub fn cp_a_imm8(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let value = mmu.read_byte(self.pc, timer);
        self.alu_cp(value);
        self.pc = self.pc.wrapping_add(1);
        self.cycles = self.cycles.wrapping_add(4);
    ...
    /// Incrementa PC di 1.
    pub fn ldh_mem8_a(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let offset = mmu.read_byte(self.pc, timer) as u16;
        self.pc = self.pc.wrapping_add(1);
        mmu.write_byte(0xFF00 | offset, self.a, timer);
        self.cycles = self.cycles.wrapping_add(12);
    }
    ...
    /// Usa la memoria alta I/O (0xFF00-0xFFFF).
    /// Incrementa PC di 1.
    pub fn ldh_a_mem8(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let offset = mmu.read_byte(self.pc, timer) as u16;
        self.pc = self.pc.wrapping_add(1);
        self.a = mmu.read_byte(0xFF00 | offset, timer);
        self.cycles = self.cycles.wrapping_add(12);
    }
    ...
    pub fn ld_mem_c_a(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let addr = 0xFF00 | (self.c as u16);
        mmu.write_byte(addr, self.a, timer);
        self.cycles = self.cycles.wrapping_add(8);
    }
    ...
    /// Versione dinamica di LDH usando il registro C.
    pub fn ld_a_mem_c(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let addr = 0xFF00 | (self.c as u16);
        self.a = mmu.read_byte(addr, timer);
        self.cycles = self.cycles.wrapping_add(8);
    }
    ...
    /// Incrementa PC di 2.
    pub fn ld_mem16_a(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let low = mmu.read_byte(self.pc, timer) as u16;
        self.pc = self.pc.wrapping_add(1);
        let high = mmu.read_byte(self.pc, timer) as u16;
        self.pc = self.pc.wrapping_add(1);
    ...
    /// Incrementa PC di 2.
    pub fn ld_a_mem16(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let low = mmu.read_byte(self.pc, timer) as u16;
        self.pc = self.pc.wrapping_add(1);
        let high = mmu.read_byte(self.pc, timer) as u16;
        self.pc = self.pc.wrapping_add(1);
    ...
    /// Azzera Z e N. Modifica H e C basati sull'operazione a 8-bit.
    /// Incrementa PC di 1.
    pub fn ld_hl_sp_e8(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let raw_offset = mmu.read_byte(self.pc, timer);
        self.pc = self.pc.wrapping_add(1);
    ...
    pub fn cb(&mut self, mmu: &mut Mmu, timer: &mut Timer) {
        let opcode = mmu.read_byte(self.pc, timer);
        self.cb_prefixed(opcode, mmu, timer);
        self.pc = self.pc.wrapping_add(1);
        self.cycles = self.cycles.wrapping_add(8);
    }
}
